#' Plot Spatial Precipitation Distribution
#'
#' This function takes a NetCDF file containing daily precipitation time series, extracts spatial and temporal features,
...
#'                  "daily" provides the nonzero average of daily precipitation during the whole period given in the NC file.
#'                  "monthly" provides the average total precipitation within every month during the whole duration.
#'                  "annual" provides the annual maxima of daily precipitation during the whole duration.
#' @return A ggplot object showing the specified precipitation distribution over space.Note: We are using the default "Virdis"
...
plot_precip_map <- function(nc_file, stat_type = c("daily", "monthly", "annual")) {
  ...
  if (stat_type == "daily") {
    daily_nz <- sf_obj %>%
      rowwise() %>%
      mutate(non_zero_mean = mean(c_across(all_of(cols)) != 0, na.rm = TRUE)) %>%
      ungroup() %>%
      select(non_zero_mean, geometry)
    
    stars_daily_nz <- st_rasterize(daily_nz["non_zero_mean"])
    
    p <- ggplot() +
      geom_stars(data = stars_daily_nz) +
      coord_equal() +
      scale_fill_viridis_c(
        "Nonzero Average Daily Precipitation (mm)",
        direction = -1,
    ...
    if (stat_type == "monthly") {
      ...
      p <- ggplot() +
        ...
        coord_equal() +
        scale_fill_viridis_c(
          "Annual Maxima of daily precipitation (mm)",
          direction = -1,
          na.value = "transparent"
        ) +
        theme_minimal() + xlab("Longitude") + ylab("Latitude") + mytheme +
