...
Plot the basin daily precipitation values for the basin

```{r, eval=FALSE}
plot(values)
```

![](precip.png){width="10cm"}

## Visualization functions

we introduce three essential functions used to generate precipitation time series, seasonal and monthly precipitation distributions, and probability exceedance plots. 

0. Load the data. 
This data should be in dataframe format that contains two columns, named "date" and "precipitation"

```{r}
fpath_df <- system.file("extdata", "eg_TS.rds", package = "CMIP6VisR")
data <- readRDS(fpath_df)
